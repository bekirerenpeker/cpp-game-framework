...
- **UI rendering** ([graphics/ui/UiRenderer.hpp](engine/include/graphics/ui/UiRenderer.hpp)): the Singleton `UiSystem::draw()` hands the solved tree to. **The UI draws in window space, not world space** — `init(shader)` gives it its own `BatchRenderer<UiVertex>` over `UiShader.glsl`, and `setViewport(screenTopLeft, pixelsPerUiUnit)` places a root in window pixels, so the camera does not move or scale it. Every container is **one quad**: the fragment shader evaluates a rounded-box signed distance field, giving `cornerRadius`, `borderWidth`/`borderColor` and antialiased edges for free, with no texture bound at all (hence `BatchRenderer::nextQuad()`, the texture-less overload — a texture-less shader declares no `uTextures`, and looking that up would warn on every flush). A container whose background and border are both invisible emits **no geometry**, which is what makes layout-only containers free — and most containers are layout-only. Rects and glyphs are separate batches, so the rect batch always flushes before any glyph is submitted; text is put into the same space via `TextRenderer::setViewProjOverride`, which flushes on change (submit all world text, then all UI text). **Only `TextStyle::size` is scaled from UI units to pixels** — every other measurement in a `TextStyle` is in em and therefore scales itself, which is the payoff of the em rule. There is no debug-box pass: styles are the only thing drawn.
