...
### Other subsystems
- **Logging** ([core/logging](engine/include/core/logging)): use `LOG_INFO/LOG_WARNING/LOG_ERROR(fmt, ...)` (std::format-style). Sinks are pluggable (`ConsoleSink`, `FileSink`); add with `Logger::get().addSink<FileSink>(path)`. `setUseAsync(true)` enables async logging. Register a `std::formatter` for a custom type via the `DEFINE_TYPE_FORMATTER` macro.
...
- Don't overuse comments — comment only what the code can't say itself (non-obvious intent, magic numbers, tricky invariants). Never restate what a variable or parameter does; put that in the name. Names must be descriptive but not long.
