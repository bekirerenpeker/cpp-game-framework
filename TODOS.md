...
- [x] **Character controller in `physics_test`** — scene-side, not an engine type. Proportional
  ...
  world units, ground from a downward box cast. Settled the input-ordering question: latch in
  `onWindowUpdate` where input is fresh, consume in `onFixedUpdate`, leave the loop order alone.
