...
class EntityHandle
{
  ...
  public:
    ...
    {
        return m_registry.getPool<T>().insert(m_entity, T {std::forward<Args>(args)...});
    }
    ...
    template<typename... T> bool containsAll() { return (contains<T>() && ...); }
    template<typename... T> bool containsSome() { return (contains<T>() || ...); }

    template<typename T> T* tryGet() { return contains<T>() ? &get<T>() : nullptr; }
    template<typename T> const T* tryGet() const { return contains<T>() ? &get<T>() : nullptr; }
